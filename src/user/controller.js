...
//     if (email === 'clad3d@gmail.com') {
//       user = await checkAdminNameandEmail(email);
//     } else {
//       user = await checkUserNameandEmail(email);
//     }
...
// 📥 Get all users for a specific company (excluding Super Admins)
...
// 🔍 Get single user profile by ID with role and company info
...
// ✏️ Update user and company profile (with optional image upload to AWS S3)
// // controllers/userController.js
// const validateEmail = (email) => /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email);
// const validatePhoneNumber = (number) => /^\+?[1-9]\d{6,14}$/.test(number);
...
const deleteUserPermanent = async (req, res) => {
  ...
  try {
    ...
    // Optional: Fetch user to delete uploaded files
    const userResult = await client.query("SELECT profile_img, company_logo FROM users WHERE id = $1", [userId]);
    const user = userResult.rows[0];

    if (user) {
      const filesToDelete = ["profile_img", "company_logo"];
      filesToDelete.forEach((field) => {
        if (user[field]) {
          const filePath = path.join(__dirname, "../../uploads", user[field]);
          if (fs.existsSync(filePath)) fs.unlinkSync(filePath);
        }
      });
    }

    // Delete all related tables in a transaction
    const deleteQueries = [
      { query: "DELETE FROM doors_logs WHERE user_id = $1", params: [userId] },
      { query: "DELETE FROM doors WHERE user_id = $1", params: [userId] },
      { query: "DELETE FROM doors_cal WHERE user_id = $1", params: [userId] },
      { query: "DELETE FROM customer_info WHERE user_id = $1", params: [userId] },
      { query: "DELETE FROM user_subscriptions WHERE user_id = $1", params: [userId] },
      { query: "DELETE FROM forgot_otp WHERE user_id = $1", params: [userId] },
      { query: "DELETE FROM transactions WHERE user_id = $1", params: [userId] },
      { query: "DELETE FROM company WHERE created_by = $1 OR updated_by = $1", params: [userId] },
      { query: "DELETE FROM users WHERE id = $1", params: [userId] },
    ];

    for (const dq of deleteQueries) {
      await client.query(dq.query, dq.params);
    }
    ...
    return res.status(500).json({ message: "❌ Failed to delete user.", status: false, error: err.message });
  } finally {
    client.release();
  }
};
